...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        # configurando a camada oculta aleatória
        # reservamos 2*d dimensões para os dados originais e os quadrados.
        # o restante necessário até chegar ao limite de 64 vai ser preenchido pelos neurônios na relu
        self.k = self.DIM_MAX - (2 * d)

        # usando uma seed fixa para garantir o determinismo
        g = torch.Generator().manual_seed(102)
        self.W = torch.randn(d, self.k, generator=g)
        self.b = torch.randn(self.k, generator=g)

    def phi(self, X: torch.Tensor) -> torch.Tensor:
        Xs = (X - self.mu) / self.sd

        # mapeamento geométrico de quadrados que resolve esferas e círculos
        quadrados = Xs ** 2

        # mapeamento geométrico para camada oculta simulada com ReLU que
        # resolve as formas espiral, luas e XOR através da dobra e segmentação do espaço
        ativacao_relu = torch.relu(Xs @ self.W + self.b)

        # concatena as entradas originais, quadrados e os neurônios ReLU, onde:
        # d + d + k = 64 dimensões
        return torch.cat([Xs, quadrados, ativacao_relu], dim=1)
